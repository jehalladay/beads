...
func runCreateProxiedServer(cmd *cobra.Command, ctx context.Context, in createInput) error {
	if in.repoOverrideSet {
		return fmt.Errorf("--repo is not supported with --proxied-server")
	}
	switch {
	case in.graphFile != "":
...
func proxiedOpenUOW(ctx context.Context) (uow.UnitOfWork, domain.CreateContext, error) {
	if uowProvider == nil {
		return nil, domain.CreateContext{}, fmt.Errorf("proxied-server UOW provider not initialized")
	}
	uw, err := uowProvider.NewUOW(ctx)
	if err != nil {
		return nil, domain.CreateContext{}, fmt.Errorf("open unit of work: %v", err)
	}
	cctx, err := uw.ConfigUseCase().LoadCreateContext(ctx)
	if err != nil {
		uw.Close(ctx)
		return nil, domain.CreateContext{}, fmt.Errorf("load create context: %v", err)
	}
	return uw, cctx, nil
}
...
func runCreateProxiedSingle(_ *cobra.Command, ctx context.Context, in createInput) error {
	...
	if in.dryRun {
		...
		if in.parentID != "" {
			...
			if err != nil {
				return fmt.Errorf("open unit of work: %v", err)
			}
			if _, err := dryUW.IssueUseCase().GetIssue(ctx, in.parentID); err != nil {
				dryUW.Close(ctx)
				return fmt.Errorf("parent issue %s not found: %v", in.parentID, err)
			}
			if !in.noInheritLabels {
				inherited, lerr := dryUW.LabelUseCase().GetLabels(ctx, in.parentID)
				if lerr != nil {
					dryUW.Close(ctx)
					return fmt.Errorf("dry-run inherit labels: %v", lerr)
				}
...
func runCreateProxiedMarkdown(_ *cobra.Command, ctx context.Context, in createInput) error {
	...
	if err != nil {
		return fmt.Errorf("parsing markdown file: %v", err)
	}
	if len(templates) == 0 {
		return fmt.Errorf("no issues found in markdown file")
	}
	...
	if in.validationMode == "error" || in.validationMode == "warn" {
		for _, t := range templates {
			...
			if err := validation.LintIssue(lintIssue); err != nil {
				if in.validationMode == "error" {
					return fmt.Errorf("template %q: %v", t.Title, err)
				}
	...
	for _, b := range builds {
		...
		if !b.template.IssueType.IsValidWithCustom(customTypes) {
			return fmt.Errorf("template %q: invalid type %q", b.template.Title, b.template.IssueType)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("creating issues from markdown: %v", err)
	}
	...
	if in.jsonOutput {
		return outputJSON(result.Issues)
	}
...
func runCreateProxiedGraph(_ *cobra.Command, ctx context.Context, in createInput) error {
	...
	if err != nil {
		return fmt.Errorf("reading graph plan: %v", err)
	}
	...
	if err := json.Unmarshal(data, &plan); err != nil {
		return fmt.Errorf("parsing graph plan: %v", err)
	}
	...
	if in.dryRun {
		...
		dryUW.Close(ctx)
		if err != nil {
			return fmt.Errorf("load create context: %v", err)
		}
		if err := validateGraphApplyPlan(&plan, resolveProxiedCustomTypes(cctx.CustomTypes)); err != nil {
			return fmt.Errorf("invalid graph plan: %v", err)
		}
	...
	if err != nil {
		return fmt.Errorf("graph create: %v", err)
	}
	...
	if in.jsonOutput {
		return outputJSON(GraphApplyResult{IDs: result.IDs})
	}
...
func materializeGraphNodeIssue(n GraphApplyNode, in createInput) (*types.Issue, error) {
	...
	if len(n.Metadata) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("node %q: marshaling metadata: %v", n.Key, err)
		}
		metadataJSON = raw
	}
	return &types.Issue{
