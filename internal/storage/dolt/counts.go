...
// CountDependents returns the number of blocks-type edges that depend on
// issueID (both dependency tables), so the scalar dependent_count matches
// list/ready (GetDependencyCountsInTx also filters type='blocks'). beads-1d3i:
// this count feeds only the `bd show --json` scalar; parent-child hierarchy
// links are NOT dependents in the blocker sense and must not inflate the
// readiness/blocker signal. A dependent may be a permanent issue (edge in
// `dependencies`) or a wisp (edge in `wisp_dependencies`, routed there by
// WispTableRouting on the source).
//
...
// CountDependencies returns the number of blocks-type edges that issueID
// depends on (both dependency tables), so the scalar dependency_count matches
// list/ready (GetDependencyCountsInTx also filters type='blocks'). beads-1d3i:
// this count feeds only the `bd show --json` scalar; parent-child hierarchy
// links are NOT dependencies in the blocker sense and must not inflate the
// readiness/blocker signal. A wisp's outgoing edges live in `wisp_dependencies`,
// a permanent issue's in `dependencies`. Counted as two separate queries summed
// in Go (see CountDependents for why a single combined query is avoided).
